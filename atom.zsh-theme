# Atom theme for oh-my-zsh
# Tested on Linux, Unix and Windows under ANSI colors.
# Based on original Zeta theme (from Skyler Lee, 2015)
# Copyright: Terry Finkel, 2019
...
function get_prompt_indicator {
    if [[ $? -eq 0 ]]; then
        echo "%{$blue%}$atom %{$reset_color%}"
    else
        echo "%{$red_bold%}$atom %{$reset_color%}"
    fi
}
